...
class Lamas extends StorageManager {
	...
	public function newsValidGet(){
		$this->dbConnect();
		$requete = "SELECT * FROM `lamas` WHERE online=1 ORDER BY `date_news` DESC" ;
		//print_r($requete);
		$new_array = null;
	...
	public function newsGet($id){
		 $this->dbConnect();
		if (!isset($id)){
			$requete = "SELECT * FROM `lamas` ORDER BY date_news DESC" ;
		} else {
			$requete = "SELECT * FROM `lamas` WHERE id_news=". $id ;
		}
		//print_r($requete);
		$new_array = null;
		$result = mysqli_query($this->mysqli,$requete);
