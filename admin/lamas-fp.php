	...
	if ($_POST['reference'] == 'lamas'){
		...
		for ($i=1;$i<2;$i++){
			...
			if(strstr($source,'uploads')){
				...
				//Image
				$destination=$_SERVER['DOCUMENT_ROOT'].'/photos/lamas'.$filenameDest;
				//print_r($destination);exit();
				$imageManager->imageResize($source, $destination, null, 650, false);
				//Vignette
				$destination=$_SERVER['DOCUMENT_ROOT'].'/photos/lamas/thumbs'.$filenameDest;
		...
		if ($_POST['action'] == 'modif') { //Modifier la news
			try {
				...
				header('Location: /admin/lamas-list.php');
			} catch (Exception $e) {
			...
			try {
				...
				header('Location: /admin/lamas-edit.php?id='.$result);
			} catch (Exception $e) {
				echo 'Erreur contactez votre administrateur <br> :',  $e->getMessage(), "\n";
				exit();
			}
