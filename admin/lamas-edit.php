...
if (!empty($_GET)){ //Modif 
	...
	if (empty($result)) {
		...
		for ($i=1;$i<2;$i++) {
			...
			if(empty($image[$i]) || !isset($image[$i])){
				$img[$i]  = '/admin/img/favicon.png';
				$imgval[$i]  = '';
			} else {
				$img[$i]  = '/photos/lamas/thumbs'. $image[$i];
				$imgval[$i]  = $image[$i];
			}
		}
	}
	...
	$action = 'add';
	$labelTitle = 'Edition Nouvelle vente de lama ';
	$id_produit= 			null;
	$titre=  		null;
	$date_news= 	null;
	$accroche= 		null;
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-xs-12 col-sm-12 col-md-12">
				<form name="formulaire" class="form-horizontal" method="POST"  action="lamas-fp.php">
					<input type="hidden" name="reference" value="lamas">
					...
					<div class="form-group" >
					 ...
					 <div class="form-group" >
						<label for="titre">Lama en vente :</label>
					...
					<div class="form-group">
						<label for="accroche">Texte / Description :</label><br>
					...
					<script type="text/javascript">
						...
						function clearImage(idImage){
							$('#customRoxyImage'+idImage).attr('src', '/admin/img/favicon.png');
							$('#url'+idImage).val('');
						}
